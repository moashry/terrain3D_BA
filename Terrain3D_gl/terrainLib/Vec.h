...
namespace tum3D
{
	...
	Mat<4,4,T> &perspectiveOffCenterProjMatGL(T left, T right, T bottom, T top, T zNear, T zFar, Mat<4,4,T> &m)
	{
		...
		m.get(3,2) = T(2.0) * zNear * zFar / (zNear - zFar);
		m.get(2,3) = T(-1.0);
	...
	template <class T>
	Mat<4,4,T> &perspectiveOffCenterProjMat(T left, T right, T bottom, T top, T zNear, T zFar, Mat<4,4,T> &m)
	{
		m.clear();
		m.get(0,0) = T(2.0) * zNear / (right - left);
...
*/
	...
	typedef Vec3<unsigned char> Vec4uc; ///< four-dimensional unsigned char vector
